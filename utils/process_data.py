...
#   data_gene = zip(text_data.weak_label, text_data.strong_label, text_data.paragraph)

#   for weak, strong, text in data_gene:
...
# tokenizer = AutoTokenizer.from_pretrained(DEFAULT_MODEL_NAME)
# s_d = train_data['para'][0][0][0]
# len(tokenizer(s_d, add_special_tokens = False).input_ids)
# for tmp, blk_chose in d:
#   tmp += '[SEP]'
# tokenizer.decode(tokenizer(tmp, add_special_tokens = False).input_ids)
# tokenizer.tokenize(tmp)
# len(tmp)
# train_data[1][0][9]
# para = train_data
